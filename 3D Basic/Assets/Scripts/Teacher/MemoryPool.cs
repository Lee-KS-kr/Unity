...
{
    private int _currentSize = 0; //큐의 현재 크기 + 새로 생성되는 적 이름에 붙일 인덱스 번호로도 싸용
    private Queue<GameObject> _pool = null; // 대량으로 생성해 놓은 메모리 풀

    public GameObject objectPrefabs = null; // 생성할 오브젝트의 종류
    public Transform parent = null;
    public int poolSize = 8;

    private static MemoryPool _instance = null; // 싱글톤은 나중에 제거할 부분
    ...
    {
        _pool = null; // 적 종류의 수만큼 배열 크기 확정
        _currentSize = 0; //각 풀의 크기값 초기화
    ...
    {
        _pool = new Queue<GameObject>(targetSize); // 비어있는 풀 생성. 초기 크기 지정
        ...
        for (int i = 0; i < makeSize; i++) // 오브젝트 하나씩 생성
        {
            ...
            obj.name = $"{objectPrefabs.name}_{_currentSize}"; //이름 변경
            obj.SetActive(false); //오브젝트 비활성화

            _pool.Enqueue(obj); //큐에 오브젝트 삽입
            _currentSize++; //currentSize 증가
        }
    }
    ...
    public GameObject GetObject()
    {
        ...
        if (_pool.Count <= 0)
        {
            ...
            PoolExpand(_currentSize << 1); // 큐가 두배로 커지고 오브젝트도 추가되는 함수
        }
        
        // 큐에 오브젝트가 있다.
        result = _pool.Dequeue(); //Dequeue를 통해 오브젝트 하나 꺼냄
